...
void RangeSlider::paintEvent(QPaintEvent* aEvent)
{
    ...
    QPen pen(Qt::gray, 0.3);
    painter.setPen(pen);
    painter.setRenderHint(QPainter::Qt4CompatiblePainting);
    QBrush backgroundBrush(QColor(121, 121, 121));
    painter.setBrush(backgroundBrush);
    painter.drawRoundedRect(backgroundRect, 11, 11);
    ...
    QBrush handleBrush(Qt::gray);//dddddddddddddddddddddddddddddddddddd
    painter.setBrush(handleBrush);
    QRectF leftHandleRect = firstHandleRect();
    if(type.testFlag(LeftHandle))
        painter.drawRoundedRect(leftHandleRect, 5,5);

    // Second value handle rect
    QRectF rightHandleRect = secondHandleRect();
    if(type.testFlag(RightHandle))
        painter.drawRoundedRect(rightHandleRect, 5,5);
...
void RangeSlider::mousePressEvent(QMouseEvent* aEvent)
{
    if(aEvent->buttons() & Qt::LeftButton)
    {
        ...
        if(posCheck >= 2
            && posCheck <= posMax - 2)
        {
            int step = mInterval / 25 < 1 ? 1 : mInterval / 25;
...
void RangeSlider::SetTracking(bool enable) {
